        ...
        describe("TokenProvider") {
            ...
            describe("getAllClaimsFromToken") {
                it("should return claims from a valid token") {
                    // Given
                    val userId = 1L
                    val token = tokenProvider.createTokens(userId, listOf("ROLE_USER")).accessToken
            ...
            describe("getphoneNumberFromToken") {
                it("should return phoneNumber from token") {
                    ...
                    // When
                    val extractedUserId = tokenProvider.getUserIdFromToken(token)
            ...
            describe("getIdFromExpiredToken") {
                it("정상 토큰으로 ID 추출") {
                    ...
                    // When
                    val extractedUserId = tokenProvider.getIdFromExpiredToken(token)
            ...
            describe("getRolesFromToken") {
                it("should return roles from token") {
                    ...
                    val token =
                        tokenProvider.createAccessToken(userId, listOf("ROLE_USER", "ROLE_ADMIN"))
            ...
            describe("tokenReissue") {
                it("should reissue a new access token if refresh token is valid") {
                    ...
                    val tokens = tokenProvider.createTokens(userId, roles)
                    val accessToken = tokens.accessToken
                    val refreshToken = tokens.refreshToken

                    every { redisRepository.getValue("RT:$userId") } returns refreshToken
            ...
            describe("validateToken") {
                it("should return true for a valid token") {
                    // Given
                    val token = tokenProvider.createTokens(1L, listOf("ROLE_USER")).accessToken
            ...
            describe("createAccessToken") {
                it("should create a valid access token") {
                    ...
                    // When
                    val token = tokenProvider.createAccessToken(userId, listOf(roles))

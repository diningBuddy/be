...
class HomeService(
    ...
    private val categoryService: GetCategoryService,
    private val popularRestaurantService: GetPopularRestaurantService
) {
    companion object {
    ...
    fun getHome(
        ...
        val (lunchRequest, midNightRequest) = sectionConditions(request)
        ...
        val popularRestaurants = popularRestaurantService.getPopularRestaurants(
            PageRequest.of(0, 20)
        )
        return HomeResponse(
            ...
            restaurantCategories = listOf(
                categoryService.getCategories()
            )
        )
    }

    private fun sectionConditions(request: HomeRequest): Pair<GetRestaurantsRequest, GetRestaurantsRequest> {
        ...
        return Pair(lunchRequest, midNightRequest)
    }
}
